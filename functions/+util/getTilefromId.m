function [Io,filename] = getTilefromId(scopeloc,idx)
% given tile id returns 
filecent = scopeloc.filepath{idx};
basename = strsplit(filecent,'/');
filename = fullfile(filesep,basename{1:end-1},sprintf('%s-ngc.0.tif',basename{end-1}));
