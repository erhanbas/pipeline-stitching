function skel_unbinaryfile(data)
%
% If the data file you selected in "main function" is unbinary, this function will be called.
%
% input:
...
skel_show3D(XX,YY,ZZ,voxel,0.5,3,AZ,EL,1);
title('original data');
...
if isempty(decision)
    ...
    if isempty(str_size)
        ...
        v=voxel_eroded;
        voxel=v;
        skel_show3D(XX,YY,ZZ,voxel,0.5,4,AZ,EL, 1);
...
if isempty(decision)
    ...
    voxel=voxel2;
    voxel((voxel>0))=1;
    skel_show3D(XX,YY,ZZ,voxel,0.5,5,AZ,EL, 1);    
    title('Filtered data');
    if rev==1
        set(gca, 'ZDir','reverse');
    end
...
switch method
    case 1,
        ...
        skel_Distmethod(voxel, show, rows, cols, slices, XX, YY, ZZ, AZ, EL, rev);
    case 2,
        disp('You have chosen Repulsive Potential Field Method.');
        skel_RPFmethod(voxel, show, rows, cols, slices, XX, YY, ZZ, AZ, EL, rev);
    case 3,
        disp('You have chosen Thinning Method.');
        skel_thinningmethod(voxel, show, rows, cols, slices, XX, YY, ZZ, AZ, EL, rev);
end
