function [T, cost] =  minispanningtree(CostMatrix)
% Kruskal's algorithm for Minimum Spanning Tree
%
% input:
% CostMatrix is a distance matrix of all pair of nodes for inspecting connection
%
% output:
...
%
% ---------------------------
% modified by Li Liu in 10/30/2012 
% l.liu6819@gmail.com
...
if n<2
    disp(' ');
    disp('There should be at least two points for spanning tree connection!');
    disp(' ');
end
...
for i = 1:n
    for j = 1:n
        if  CostMatrix(i,j)~=inf
            ...
            EdgeWeights(EdgeWeightsCounter,1) = CostMatrix(i,j);    %give the edge weight from CostMatrix(i,j) to the first column of the EdgeWeights matrix
            EdgeWeights(EdgeWeightsCounter,2) = i;                  %give the left vertice connected by the edge to the second column of the EdgeWeights matrix
            EdgeWeights(EdgeWeightsCounter,3) = j;                  %give the right vertice connected by the edge to the third column of the EdgeWeights matrix
        end
    end
end

SortedEdgeWeights = 0;
SortedEdgeWeights = sortrows(EdgeWeights);                          %sorting the edge weight
...
while (MSTreeEdgesCounter < (size(MSTreeEdges,1)))
    MSTreeEdgesCounter = MSTreeEdgesCounter + 1;
