function [Vn] = ordfilt3D(V0,ord,padoption);
...
elseif isa(V0,'uint16')
    V = uint16(padarray(V0,[1 1 1],padoption));
    S = size(V);
    Vn = uint16(zeros(S(1),S(2),S(3),26));  % all the neighbor
else
...
Vn = sort(Vn,4);
% eB: only keep the output channels
Vn(:,:,:,setdiff(1:27,ord)) = [];
