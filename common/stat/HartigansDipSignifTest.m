function [dip, p_value, xlow,xup]=HartigansDipSignifTest(inputvec,nboot,varargin)

p_value = zeros(1,size(inputvec,2));
for idim =1:size(inputvec,2)
    [xpdf, n, b] = compute_xpdf(inputvec(:,idim));
    [dip, p_value(idim), xlow, xup] = HartigansDipSignif(xpdf,500);
end
end
function [dip, p_value, xlow,xup]=HartigansDipSignif(xpdf,nboot,varargin)
...
x=sort(xpdf(:));
N=length(x);
[dip,xl,xu, ifault, gcm, lcm, mn, mj] = deal([]);
mn=zeros(size(x));
mj=zeros(size(x));
lcm=zeros(size(x));
gcm=zeros(size(x));
...
if (N<=0);ifault=1;
elseif (N==1);xl=x(1);xu=x(N);dip=0.0;ifault=2;
elseif (N>1)
    % Check that X is sorted
    if (x ~= sort(x)); ifault=3;    end
